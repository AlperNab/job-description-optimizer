...
**Domain:** `HR / Hiring Marketing` · **Suite:** `HR & Recruiting Suite` · **Accent:** `#fbbf24`
...
**Job Description Optimizer** is a standalone, browser-based AI workflow app for **HR / Hiring Marketing**. It turns structured inputs, uploaded files, and project-specific settings into reviewable outputs using a deterministic local engine plus optional local/cloud LLM enhancement.

**Core job:** Draft JD → optimized job description.
...
|---|---:|:---:|---|---|
| `draft_jd`<br>Draft JD | text | Yes | — | Affects input: Draft JD. |
...
| `role_family`<br>role family | select | No | — | Affects customization: role family. |
| `seniority`<br>seniority | select | No | intern | Affects customization: seniority. |
| `location`<br>location | text | No | — | Affects customization: location. |
| `remote_hybrid`<br>remote/hybrid | text | No | — | Affects customization: remote/hybrid. |
| `tone`<br>tone | text | No | — | Affects customization: tone. |
| `required_nice_to_have_ratio`<br>required/nice-to-have ratio | text | No | — | Affects customization: required/nice-to-have ratio. |
| `legal_constraints`<br>legal constraints | text | No | — | Affects customization: legal constraints. |
...
uvicorn app.main:app --host 127.0.0.1 --port 9129
```
...
| `AI_SUITE_PORT` | Port for this project GUI, default `9129`. |
...
```bash
curl http://127.0.0.1:9129/api/health
```
...
```bash
curl -X POST http://127.0.0.1:9129/api/run \
...
```bash
AI_SUITE_HOST=0.0.0.0 AI_SUITE_PORT=9129 uvicorn app.main:app --host 0.0.0.0 --port 9129
```
...
- `plugins/job-description-optimizer.json` for plugin metadata.
